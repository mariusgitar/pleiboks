...
async function fetchSpotifyCovers(tracks) {
  const results = await Promise.all(
    tracks.map((t) =>
      fetch(`/api/spotify-track?id=${t.id}`)
...
function PauseIcon({ size = 22, color = "#fff" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <rect x="5" y="4" width="4.5" height="16" rx="2" fill={color} />
      <rect x="14.5" y="4" width="4.5" height="16" rx="2" fill={color} />
    </svg>
  );
}
...
function PrevIcon({ size = 22, color = "rgba(255,255,255,0.75)" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <polygon points="13,5 3,12 13,19" fill={color} />
      <rect x="14" y="5" width="5" height="14" rx="1.5" fill={color} />
    </svg>
  );
}
...
function NextIcon({ size = 22, color = "rgba(255,255,255,0.75)" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <polygon points="11,5 21,12 11,19" fill={color} />
      <rect x="5" y="5" width="5" height="14" rx="1.5" fill={color} />
    </svg>
  );
}
...
function GlowRing({ size, radius, mini = false }) {
  ...
  const g = mini ? 4 : 10;
  const grad =
    "linear-gradient(135deg,#FF6B6B,#FF9F45,#FFD93D,#6BCB77,#4D96FF,#845EC2,#FF6B6B)";
    ...
    >
      <div
        style={{
          position: "absolute",
          inset: 0,
          background: "linear-gradient(160deg,rgba(255,255,255,0.07) 0%,rgba(0,0,0,0.28) 100%)",
          pointerEvents: "none",
        }}
      />
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", textAlign: "left", marginBottom: "auto" }}>
        <div style={{ color: "#fff", fontSize: "1.5rem", fontWeight: 900, lineHeight: 1.2, marginBottom: 4 }}>
          {item.title}
        </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        overflow: "hidden",
        background: "#fff",
        border: `3px solid ${isActive ? section.accent : "transparent"}`,
            ...
            {[0, 0.2, 0.1].map((d, i) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 2,
                  background: "#fff",
                  animation: `eqbar 0.65s ease-in-out ${d}s infinite alternate`,
                }}
              />
            ))}
          </div>
...
export default function App() {
  ...
  async function togglePlay() {
    if (source === "nrk") {
      ...
      if (playing) {
        await spotifyPause(auth.accessToken);
        setPlaying(false);
      } else {
        await spotifyPlay(activeItem.uri, auth.accessToken);
        setPlaying(true);
      }
    }
  }
  ...
  // ── Desktop/iPad-spiller: flytende, sticky og skjermhøy ────────
  const IpadPlayer = () => {
      ...
      >
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: "linear-gradient(160deg,rgba(255,255,255,0.06) 0%,rgba(0,0,0,0.18) 100%)",
            pointerEvents: "none",
          }}
        />
        ...
        >
          ...
          <div style={{ minWidth: 0, paddingTop: 8 }}>
            <div style={{ marginBottom: 20 }}>
              ...
              <p style={{ fontSize: "0.78rem", fontWeight: 700, color: "#1B4D5C", opacity: 0.5, margin: 0 }}>
                Hva vil du høre?
              </p>
            </div>
            <Katalog />
        ...
        <>
          ...
          <div style={{ position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", padding: "0 0 120px" }}>
            <div style={{ padding: "3rem 1.1rem 1rem" }}>
              ...
              <p style={{ fontSize: "0.8rem", fontWeight: 700, color: "#1B4D5C", opacity: 0.5, margin: 0 }}>
                Hva vil du høre?
              </p>
            </div>
          ...
          {activeItem && !fullscreen && (
            ...
            >
              <CoverImg
                item={activeItem}
                sectionColor={activeSection?.color || "#E3F2F9"}
                size={50}
                radius={12}
                playing={playing}
                onClick={(e) => {
